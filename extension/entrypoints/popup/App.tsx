...
export function App() {
  ...
  const [view, setView] = useState<'task' | 'proof' | 'ledger'>('task');
  ...
  function confirm(confirmed: boolean) {
    ...
    if (actionId) void command({ type: 'CONFIRM_ACTION', actionId, requestId: agent.pendingConfirmation?.requestId, confirmed });
  }
  ...
  return <main className={judgeView ? 'judge-proof-page' : 'extension-shell'}>
    ...
    {judgeView ? <><p className="view-note">{frozen ? 'Display frozen for presenting. The agent itself is not paused.' : 'Review after the run. Keep the target website active while the agent works.'}</p>{uiError && <p role="alert">{uiError}</p>}<PrivacyProof agent={agent} expanded /></> : <>
      <nav className="view-switch" aria-label="Extension views"><button aria-pressed={view === 'task'} onClick={() => setView('task')}>Agent</button><button aria-pressed={view === 'proof'} onClick={() => setView('proof')}>Privacy proof</button><button aria-pressed={view === 'ledger'} onClick={() => setView('ledger')}>Action ledger</button></nav>
      {view === 'ledger' ? <LedgerView running={agent.running}/> : view === 'proof' ? <><div className="proof-toolbar"><span>See what was protected</span><button className="secondary" disabled={agent.running} onClick={() => void openProof()}>Open judge view <ArrowSquareOut aria-hidden="true" /></button></div><PrivacyProof agent={agent} /></> : <>
        ...
        <section className="task-card"><div className="task-label"><label htmlFor="task">What should the agent do?</label><button className="text-button" disabled={agent.running} onClick={() => setTask(DEMO_TASK)}>Use demo task</button></div><textarea id="task" rows={3} maxLength={4000} value={task} disabled={agent.running} onChange={(event) => setTask(event.target.value)} placeholder="Choose the cheapest morning fare…" />
          <div className="actions"><button className="primary" disabled={agent.running || !task.trim()} onClick={() => void command({ type: 'START_AGENT', task, rehearsal })}><Play weight="fill" aria-hidden="true" />{agent.running ? 'Agent running…' : 'Start agent'}</button><button className="stop-button" disabled={!agent.running} onClick={() => void command({ type: 'STOP_AGENT' })}><Stop weight="fill" aria-hidden="true" />Stop</button></div>
        ...
        <details className="railway-setup"><summary>Railway demo</summary><p className="hint">Use fictional data only. Loading the profile replaces existing vault values.</p><div className="actions"><button className="secondary" disabled={agent.running} onClick={() => void browser.tabs.create({ url: 'http://127.0.0.1:4173/railway.html' })}>Open railway sandbox</button><button className="secondary" disabled={agent.running} onClick={() => { setTask(RAILWAY_TASK); void command({ type: 'LOAD_DEMO_PROFILE' }); }}>Load synthetic profile</button></div><label className="rehearsal-choice"><input type="checkbox" checked={rehearsal} disabled={agent.running} onChange={(e) => setRehearsal(e.target.checked)}/> Local railway rehearsal</label><p className="hint">{rehearsal ? 'Deterministic fixture controller + DOM privacy checks. No Qwen, OCR or face inference; not a PS benchmark.' : 'Qwen-assisted railway adapter: Qwen compares sanitized fares; a local adapter handles the other steps. Full local vision/privacy checks remain active. Requires the model services; not a general-site autonomy benchmark.'}</p></details>
        <details className="vault"><summary><LockKey size={16} aria-hidden="true" /> Private values <span>{agent.secretHandles.length ? `${agent.secretHandles.length} stored` : 'Optional'}</span></summary><p className="hint">Values stay in the extension until you approve a specific fill. The destination page can then read that value; the planner receives only references.</p><div className="secret-row"><label className="sr-only" htmlFor="secret-kind">Secret type</label><select id="secret-kind" value={secretKind} onChange={(event) => setSecretKind(event.target.value)}>{SECRET_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><label className="sr-only" htmlFor="secret-value">Secret value</label><input id="secret-value" type="password" value={secretValue} onChange={(event) => setSecretValue(event.target.value)} autoComplete="off" placeholder="Stored only here" /><button className="secondary" onClick={() => void addSecret()}>Add</button></div>{agent.secretHandles.length > 0 && <div className="handles">{agent.secretHandles.map((handle) => <code key={handle}>{handle}</code>)}<button className="text-button" onClick={() => void command({ type: 'CLEAR_SECRETS' })}>Clear vault</button></div>}</details>
